...
removeNames (Named.Var name) gamma = Var index
  where
    (Just index) = elemIndex name gamma
removeNames (Named.Abs name t) gamma = Abs $ removeNames t (name : gamma)
removeNames (Named.App s t) gamma =
  App (removeNames s gamma) (removeNames t gamma)

restoreNames :: Exp -> Gamma -> Named.Exp
restoreNames (Var index) gamma = Named.Var $ gamma !! index
restoreNames (Abs t) gamma = Named.Abs name (restoreNames t $ name : gamma)
  where
    name = head $ ['a' ..] \\ gamma
restoreNames (App s t) gamma =
  Named.App (restoreNames s gamma) (restoreNames t gamma)

shift :: Exp -> Int -> Int -> Exp
shift (Var num) d cutoff =
  Var $
    if num >= cutoff
      then num + d
      else num
